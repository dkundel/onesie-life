...
const DEFAULT_POSTS = [
  {
    name: 'Dom 🐼',
    message: `<p>Fluffiness on the train
  <img src="https://pbs.twimg.com/media/DMCcRgUX4AoO3IT.jpg" alt=""></p>
  `
  },
  {
    name: 'Magic 🦄',
    message: `<p>We need more fluffiness in our lives!</p>`
  },
  {
    name: 'Best 🐱',
    message: `<p>Fluffiness around the world
  <img src="https://pbs.twimg.com/media/DMGoWIcWAAAsyxK.jpg" alt=""></p>
  `
  },
  { name: 'Dom 🐼', message: `<p>Everything is awesome! #onesiejs</p>` }
];
...
async function reset() {
  const syncList = sync.syncLists(DB_NAMES.POSTS);
  await syncList.remove();
  await sync.syncLists.create({ uniqueName: DB_NAMES.POSTS });
  for (let data of DEFAULT_POSTS) {
    await syncList.syncListItems.create({ data });
  }
  return true;
}
